...
HX_DEFINE_STACK_FRAME(_hx_pos_07f48bb0273dd92f_17_new,"flixel.graphics.tile.FlxGraphicsShader","new",0xac7b10a5,"flixel.graphics.tile.FlxGraphicsShader.new","flixel/graphics/tile/FlxGraphicsShader.hx",17,0x6817bd0c)
namespace flixel{
namespace graphics{
namespace tile{

void FlxGraphicsShader_obj::__construct(){
            	HX_STACKFRAME(&_hx_pos_07f48bb0273dd92f_17_new)
HXLINE(  18)		super::__construct(HX_("#pragma header\n\t\t\t\n\t\t\tattribute float alpha;\n\t\t\tattribute vec4 colorMultiplier;\n\t\t\tattribute vec4 colorOffset;\n\t\t\tuniform bool hasColorTransform;\n\t\t\t\n\t\t\tvoid main(void)\n\t\t\t{\n\t\t\t\t#pragma body\n\t\t\t\t\n\t\t\t\topenfl_Alphav = openfl_Alpha * alpha;\n\t\t\t\t\n\t\t\t\tif (hasColorTransform)\n\t\t\t\t{\n\t\t\t\t\topenfl_ColorOffsetv = colorOffset / 255.0;\n\t\t\t\t\topenfl_ColorMultiplierv = colorMultiplier;\n\t\t\t\t}\n\t\t\t}",5b,a9,4b,f0),HX_("#pragma header\n\t\t\t\n\t\t\tvoid main(void)\n\t\t\t{\n\t\t\t\tgl_FragColor = flixel_texture2D(bitmap, openfl_TextureCoordv);\n\t\t\t}",56,89,c9,91),false);
HXLINE(  47)		 ::flixel::graphics::tile::FlxGraphicsShader _hx_tmp = ::hx::ObjectPtr<OBJ_>(this);
HXDLIN(  47)		_hx_tmp->glFragmentHeader = (_hx_tmp->glFragmentHeader + HX_("uniform bool hasTransform;\n\t\tuniform bool hasColorTransform;\n\n\t\tvec4 flixel_texture2D(sampler2D bitmap, vec2 coord)\n\t\t{\n\t\t\tvec4 color = texture2D(bitmap, coord);\n\t\t\tif (!hasTransform)\n\t\t\t{\n\t\t\t\treturn color;\n\t\t\t}\n\n\t\t\tif (color.a == 0.0)\n\t\t\t{\n\t\t\t\treturn vec4(0.0, 0.0, 0.0, 0.0);\n\t\t\t}\n\n\t\t\tif (!hasColorTransform)\n\t\t\t{\n\t\t\t\treturn color * openfl_Alphav;\n\t\t\t}\n\n\t\t\tcolor = vec4(color.rgb / color.a, color.a);\n\n\t\t\tmat4 colorMultiplier = mat4(0);\n\t\t\tcolorMultiplier[0][0] = openfl_ColorMultiplierv.x;\n\t\t\tcolorMultiplier[1][1] = openfl_ColorMultiplierv.y;\n\t\t\tcolorMultiplier[2][2] = openfl_ColorMultiplierv.z;\n\t\t\tcolorMultiplier[3][3] = openfl_ColorMultiplierv.w;\n\n\t\t\tcolor = clamp(openfl_ColorOffsetv + (color * colorMultiplier), 0.0, 1.0);\n\n\t\t\tif (color.a > 0.0)\n\t\t\t{\n\t\t\t\treturn vec4(color.rgb * color.a * openfl_Alphav, color.a * openfl_Alphav);\n\t\t\t}\n\t\t\treturn vec4(0.0, 0.0, 0.0, 0.0);\n\t\t}",a0,78,d4,f5));
HXLINE(  85)		this->_hx___initGL();
HXLINE(  87)		this->bitmap = ( ( ::openfl::display::ShaderInput_openfl_display_BitmapData)(this->get_data()->__Field(HX_("bitmap",ef,0f,0c,f1),::hx::paccDynamic)) );
HXLINE(  88)		this->alpha = ( ( ::openfl::display::ShaderParameter_Float)(this->get_data()->__Field(HX_("alpha",5e,a7,96,21),::hx::paccDynamic)) );
HXLINE(  89)		this->colorMultiplier = ( ( ::openfl::display::ShaderParameter_Float)(this->get_data()->__Field(HX_("colorMultiplier",c4,16,81,50),::hx::paccDynamic)) );
HXLINE(  90)		this->colorOffset = ( ( ::openfl::display::ShaderParameter_Float)(this->get_data()->__Field(HX_("colorOffset",56,29,68,1a),::hx::paccDynamic)) );
HXLINE(  91)		this->hasTransform = ( ( ::openfl::display::ShaderParameter_Bool)(this->get_data()->__Field(HX_("hasTransform",b2,12,e5,d2),::hx::paccDynamic)) );
HXLINE(  92)		this->hasColorTransform = ( ( ::openfl::display::ShaderParameter_Bool)(this->get_data()->__Field(HX_("hasColorTransform",83,14,eb,f0),::hx::paccDynamic)) );
            	}
